...
class DMFNet:
    ...
    def __init__(self, struct,kernel_size,phi = np.tanh,input_size = 30, weight = None, bias = None, input_CTensor = None, input_h_mean = None, detailed_info = False, eigen_check = False):
        self.struct = struct
        ...
        # self.weight_inner[layer idx][betai][betaj][0] is the transform matrix of size [C_out, C_in]
        self.weight_inner = ['0']
        ...
        if input_CTensor is None:
            self.InitialInputCorvarianceTensor(DMFNet.rho,input_size)
        else:
            self.CTensor = input_CTensor
        # initial delta tensor
        self.UpdateDeltaTensor()
        if input_h_mean is None:
            self.InitialMeanActivation(input_size)
        else:
            self.h_mean = input_h_mean
        # initial bias, self.bias is a list of length [self.layer_len - 1] and each element is a column vector | self.bias[layer idx>=1] is a column vector 
        if bias is None:
            self.bias = ['0']
            self.InitialBias()
        else:
    ...
    def InitialMeanActivation(self,input_size = 30):
        # a list of length input_size*input_size and each element is a column vector of size [self.struct[0],1]
        self.h_mean = []
    ...
    def UpdateDeltaTensor(self):
        ...
        # alpha alpha_ are identified according to Natural Ordering
...
# Load the trained parameters and transform it to the format required by "DMFNet" class
# return [struct,weight,bias] as constructor's parameter
def DMFReadParam(fpath):
    # [Cout,Cin,kernel_size,kernel_size] for weight and [Cout,] (not column vector) for bias  
    f = open(fpath,'rb')
    ...
    for i in range(len(ParamList)):
        ...
        if ParamList[i].ndim > 2:
            lenOfCNN += 1
            struct.append((ParamList[i].shape)[0])
    if lenOfCNN > 0:
        lenOfCNN += 1 
        struct[0] = (ParamList[0].shape)[1]
    # reconstruct weight list
    if lenOfCNN == 0:
        print('Input net does not have CNN structure')
        sys.exit()
    weight = []
    weight.append('0')
    # construct the sketch
    for n in range(lenOfCNN - 1):
        C_in = struct[n]
        C_out = struct[n+1]
        weightPerLayer = [[[] for j in range(C_in)] for i in range(C_out)]
        weight.append(weightPerLayer)
    for n in range(lenOfCNN - 1):
        C_in = struct[n]
        C_out = struct[n+1]
        for cout in range(C_out):
            for cin in range(C_in):
                weight[n+1][cout][cin].append(ParamList[2*n][cout,cin,:,:])
    # reconstruct bias list
    bias = ['0']
    for n in range(lenOfCNN - 1):
        bias.append(ParamList[2*n+1].reshape(ParamList[2*n+1].shape[0],1))
    return struct, weight, bias
